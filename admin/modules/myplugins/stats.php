...
$page->add_breadcrumb_item($lang->myplugins_stats, "index.php?module=myplugins-stats");
...
if($mybb->request_method == "post") {
	...
	if($stats['code'] == "") {
		//Generate our code
		$stats['code'] = md5($mybb->settings['bburl'].random_str(50));
	}
	
	$stats['enabled'] = (bool)$mybb->input['enabled'];
	if($stats['enabled'])
		send_infos();

	$cache->update("myplugins_stats", $stats);
}

//Show our simple things...
$form = new Form("index.php?module=myplugins-stats", "post");
$form_container = new FormContainer($lang->myplugins_stats);

$check = $form->generate_check_box("enabled", true, $lang->activate, array("checked"=>$stats['enabled']));
$form_container->output_row($lang->myplugins_activate, $lang->myplugins_activate_desc, $check);

//$code_box = $form->generate_text_box("code", $code);
$code_box = "<input type=\"text\" name=\"code\" value=\"".htmlspecialchars($stats['code'])."\" class=\"text_input\" readonly=\"readonly\" />";
$form_container->output_row($lang->myplugins_code, $lang->myplugins_code_desc, $code_box);
$form_container->end();
...
function send_infos() {
	global $mybb, $cache, $plugins, $stats;
	...
	$info['mybb_version'] = $mybb->version_code;

	// What is our current charset?
	$info['charset'] = $mybb->config['database']['encoding'];
	...
	$info['plugins']['active'] = $info['plugins']['deactive'] = array();
